...
// Store verification codes temporarily (in production, use Redis or DB)
...
app.post("/sendVerificationCode", (req, res) => {
  ...
  const mailOptions = {
    ...
    to: email,
    subject: 'Sports Platform - Email Verification',
    html: `
      <div style="font-family: Arial, sans-serif; padding: 20px; background-color: #f4f4f4;">
        <div style="max-width: 600px; margin: 0 auto; background-color: white; padding: 30px; border-radius: 10px;">
          ...
          <p style="color: #888; font-size: 12px;">Sports Management Platform</p>
        </div>
      </div>
    `
  };
...
// Modified Signup with Email Verification
app.post("/signupWithVerification", (req, res) => {
  ...
  db.query(sql, [email, password, utype], (err) => {
    if (err) {
      ...
      return res.status(400).send({ message: "Email already exists or database error!" });
    }
...
app.post("/sendLoginOTP", (req, res) => {
  ...
  db.query("SELECT * FROM users WHERE emailid = ?", [emailid], (err, results) => {
    ...
    const mailOptions = {
      from: 'jitesh365days@gmail.com',
      to: emailid,
      subject: 'Sports Platform - Login OTP',
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; background-color: #f4f4f4;">
          <div style="max-width: 600px; margin: 0 auto; background-color: white; padding: 30px; border-radius: 10px;">
            ...
            <p>If you didn't request this, please ignore this email.</p>
            <hr style="margin: 20px 0;">
            <p style="color: #888; font-size: 12px; text-align: center;">Sports Management Platform</p>
          </div>
        </div>
      `
    };
...
const db = mysql.createConnection({
  host: "defaultdb-jitesh365days-b913.j.aivencloud.com",
  port: 26893,
  user: "avnadmin",
...
app.post("/picreader", async function (req, res) {
  ...
  try {
    ...
    console.error("❌ Gemini/Cloudinary error:", err);
...
app.get("/getOrganizerDetails", (req, res) => {
  db.query("SELECT * FROM organizers WHERE emailid = ?", [req.query.emailid], (err, results) => {
    if (err) {
      console.error("❌ Get Organizer Details Error:", err);
      return res.status(500).send("DB error");
    }
    res.json(results[0] || {});
  });
...
app.post("/updateOrganizerDetails", (req, res) => {
  ...
  db.query("REPLACE INTO organizers SET ?", data, (err) => {
    if (err) {
      console.error("❌ Update Organizer Details Error:", err);
      return res.status(500).send({ message: "DB error" });
    }
...
app.get('/searchTournaments', (req, res) => {
  ...
  db.query(sql, params, (err, results) => {
    if (err) {
      console.error("❌ Finder DB Error:", err);
